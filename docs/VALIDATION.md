...
- The clean directory contains no real-data manifests, retinal images/masks, model weights, logs, remote server paths, or remote launchers. The SVG under `assets/` is an original vector schematic, not a dataset sample. A basic source-pattern scan found no obvious embedded access tokens/private keys; this is not a comprehensive credential or license audit.
